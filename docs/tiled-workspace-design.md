...
The relation field is modeled in three explicit phases. First, active visible/high-salience annotation spans and focused child→parent active paths produce `RelationFieldObservation`s. Ambient visible annotations may still exert bounded residual force, but they must not define the field because ordinary scrolling in a non-focused panel should not create or remove column-scale gaps. Second, those observations solve a per-column `RelationField` over compact stack coordinates; interpolation is by compact geometry, not ordinal index. Third, layout forces are built in world coordinates: the compact prior is warped through the field, weak annotation-base forces are clamped around the field-local baseline, active visible span anchors remain high-confidence forces, focused parent context can move as a local stack, and offscreen span refinements remain bounded residual cues. This avoids ad-hoc compatibility flags and keeps weak annotation existence relations from reopening holes created by stale compact-stack coordinates.
